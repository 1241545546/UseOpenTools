...
typedef NS_ENUM(NSUInteger, LYNetworkStatusType) {
    /// 未知网络
...
/// 请求失败的Block
typedef void(^LYHttpRequestFailed)(NSError *error);
/// 缓存的Block
typedef void(^LYHttpRequestCache)(id responseCache);
 ...
 * 内修改,再加上此封装在支持CocoaPods后,如果使用者pod update最新LYNetworkHelper,那又要重新修改此
 * 封装内的相关参数.
 *
 ...
 * 了,大大降低了项目的后期维护难度
 *
 * 综上所述,最终还是将设置参数的接口暴露出来,如果通过CocoaPods方式使用LYNetworkHelper,在设置项目网络
 ...
 *
 *  @param requestSerializer LYRequestSerializerJSON(JSON格式),LYRequestSerializerHTTP(二进制格式),
 */
 ...
 *
 *  @param responseSerializer LYResponseSerializerJSON(JSON格式),LYResponseSerializerHTTP(二进制格式)
 */
